...
for i=1:length(Y1)
    ...
    f1_GOCE(i,:) = drag_force(dc,h_GOCE,[Y1(i,4);Y1(i,5);Y1(i,6)]);  %f1_GOCE=[f1, f2, f3]    
end
...
% x-Achse wird als Anzahl der Perioden gezeigt
T1 = T1 / (2*pi/n(1));
...
for i=1:length(Y1_Aero)
    ...
    f1_Aero(:,i) = drag_force(dc,h,[Y1_Aero(i,4);Y1_Aero(i,5);Y1_Aero(i,6)]); %f1_Aero=[f1, f2, f3]  
end
